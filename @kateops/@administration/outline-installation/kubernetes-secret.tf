resource "kubernetes_secret" "this_connections" {
  ...
  data = {
    ...
    REDIS_URL = "redis://${var.redis_endpoint}" #:${data.vault_generic_secret.redis.data["password"]}@${data.vault_generic_secret.redis.data["host-master"]}:${data.vault_generic_secret.redis.data["port"]}"
  }
}
